...
class MetaICLData(object):
    ...
    def __init__(self, logger=None, tokenizer_name=None, method="channel", use_demonstrations=True, k=16,
        ...
        elif 'falcon' in tokenizer_name:
            if '7B' in tokenizer_name:
                tokenizer_name = "tiiuae/falcon-7b"
            elif '40B' in tokenizer_name:
                tokenizer_name = "tiiuae/falcon-40b"
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)

        elif 'mosaic' in tokenizer_name:
            if '7B' in tokenizer_name:
    ...
    def get_dataloader(self, batch_size, is_training, calibration):
        ...
        if "labels" in inputs:
            ...
            if calibration:
                dataset = TensorDataset(inputs["input_ids"], inputs["input_ids_fake"], inputs["attention_mask"], inputs["attention_mask_fake"], inputs["token_type_ids"], inputs["token_type_ids_fake"])
            else:
    ...
    def tensorize(self, _train_data, _test_data, options=None,
        ...
        input_ids, attention_mask, token_type_ids = [], [], []
        input_ids_fake, attention_mask_fake, token_type_ids_fake = [], [], []
        metadata = []
